...
\*We use useReducer when we want to separate the state logic from the components
that make the state changes. Also when you have multiple state elements that
change together. So having multiple individual states in only one object state,
can be very helpful.

useReducer hook accept 3 params useReducer(reducerFunction, initialState,
lazyInitializerFunction)

1. reducerFunction: This is a callback which recieve the currentState AKA:
   "state" and newState value AKA: "action"

```javascript
function reducer(state, action) {}
```

2. InitialState value (usually an object)

3. lazyInitializerFunction is a function that recive the inialStateFromProps and
   return the initialState

```javascript
const lazyInitializerFunction = initialStateFromProps => initialState
```

\*Is important to understand that the reducerFunction recive the currentState
and the newValue and there are multiple ways to handle the state using this two
params, but for convencion reducerFunction implements a switch condition which
is handled by type and paylod params coming from the action prop.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=agustin.moranr%40gmail.com).
